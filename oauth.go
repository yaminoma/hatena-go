...
func (c *Consumer) getBody(method, urlStr string, oauthParams *OrderedParams) (*string, error) {

	//bodyParams := bytes.NewBufferString("scope=read_public%2Cread_private")
	//bodyParams := strings.NewReader("scope=read_public%2Cread_private")

	var jsonStr = []byte(`{"scope":"read_public,read_private"}`)
	bodyParams := bytes.NewBuffer(jsonStr)

	resp, err := c.httpExecute(method, urlStr, "", 0, bodyParams, oauthParams)
	//resp, err := c.httpExecute(method, urlStr, "", contentLength, bodyParams, oauthParams)
	if err != nil {
